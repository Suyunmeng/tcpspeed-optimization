...
skyline_detect_rtt_ms() {
  local target="1.1.1.1" rtt
  if command -v ping >/dev/null 2>&1; then
    rtt="$(LC_ALL=C ping -n -c 3 -W 2 "$target" 2>/dev/null | awk -F'[/ ]+' '/rtt|round-trip/ {print int($8+0.5); exit}')"
    if printf '%s' "$rtt" | grep -Eq '^[0-9]+$' && [ "$rtt" -gt 0 ]; then
...
skyline_write_profile() {
  ...
  # Use measured upload bandwidth as the envelope, then leave headroom for
  # retransmissions. All values are bounded so a bad speedtest cannot create an
  # unsafe BPF profile. RTT contributes to the BDP-sized cwnd ceiling.
  max_pacing=$((bandwidth * 125 / 100))
  [ "$max_pacing" -lt 50 ] && max_pacing=50
  max_pacing="$(skyline_clamp_int "$max_pacing" 50 10000)"
  bdp_packets="$(awk -v bw="$bandwidth" -v rtt="$rtt_ms" 'BEGIN { printf "%d", (bw * 1000000 / 8) * (rtt / 1000) / 1500 * 4 }')"
  max_cwnd="$(skyline_clamp_int "${bdp_packets:-4096}" 4096 50000)"
  [ "$mem_mb" -lt 1024 ] 2>/dev/null && max_cwnd="$(skyline_clamp_int "$max_cwnd" 4096 12000)"
  [ "$mem_mb" -lt 2048 ] 2>/dev/null && max_cwnd="$(skyline_clamp_int "$max_cwnd" 4096 24000)"
  queue_delay="$(skyline_clamp_int "$((rtt_ms / 2))" 50 200)"
  initial_cwnd="$(skyline_clamp_int "$((bandwidth / 2))" 32 1000)"

  if [ "$bandwidth" -le 100 ] 2>/dev/null; then
    startup_gain=2.0; cruise_inflight=1.5; cruise_pacing=1.05; loss_ratio=0.25
  elif [ "$bandwidth" -le 1000 ] 2>/dev/null; then
    startup_gain=2.5; cruise_inflight=1.8; cruise_pacing=1.08; loss_ratio=0.35
  else
    startup_gain=3.0; cruise_inflight=2.0; cruise_pacing=1.10; loss_ratio=0.5
  fi
  [ "$mem_mb" -lt 1024 ] 2>/dev/null && initial_cwnd=32

  cat > "$SKYLINE_PROFILE_FILE" <<EOF
# Generated by Speed Slayer Skyline Speeder auto-tuning.
...
skyline_apply_auto_profile() {
  ...
  local rto_floor rto_ceiling
  rto_floor="$(skyline_clamp_int "$((SKYLINE_RTT_MS * 1000))" 5000 100000)"
  rto_ceiling="$(skyline_clamp_int "$((SKYLINE_RTT_MS * 2000))" 50000 200000)"
  ...
  ssctl set-rack-rto \
    ...
    --warmup-samples 4 --rto-max-normal-permille 3000 \
    --rto-max-congested-permille 6000 \
    >>"$SKYLINE_LOG_FILE" 2>&1
  ssctl status >>"$SKYLINE_LOG_FILE" 2>&1
  success "Skyline Speeder 已应用自动参数：${SKYLINE_BANDWIDTH_MBPS}Mbps / ${SKYLINE_MEMORY_MB}MB / RTT ${SKYLINE_RTT_MS}ms"
